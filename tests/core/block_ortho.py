...
for i in range(len(basis_ref_f)):
    err2 = g.norm2(basis_ref_f[i] - basis_virtual_f[i]) / g.norm2(basis_ref_f[i])
    g.message(f"Fine vector {i}, rel. deviation = {err2}")
    assert err2 < diff_tol
...
if len(basis_ref_c[0].v_obj) == 1:
    for i in range(len(basis_ref_c)):
        err2 = g.norm2(basis_ref_c[i] - basis_virtual_c[i]) / g.norm2(basis_ref_c[i])
        g.message(f"Coarse vector {i}, rel. deviation = {err2}")
    assert err2 < diff_tol
