...
def nucleon_operator(w,u,d,x,alpha,matrix):
    ...
    return w.sum(
        u(x,alpha,a), w.sum(
            u(x,beta,b), w.epsilon(a,b,c), w.sum(
                matrix(beta,gamma), d(x,gamma,c),
                gamma
            ),
            beta, b, c
        ),
...
C_meson_2pt = g(-g.trace(prop*(C*g.gamma[5].tensor()*g.gamma[5].tensor())*g.adj(prop)*(g.gamma[5].tensor()*C*g.gamma[5].tensor())))[0,0,0,0]
