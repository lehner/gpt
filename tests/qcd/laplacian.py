...
for t, t_type in [[gpt.vspincolor, "fermion"], [gpt.mspincolor, "propagator"]]:
    ...
    for dimensions in [[0, 1, 2], [0, 1, 2, 3], [0, 1, 3]]:
        ...
        gpt.message(f"Laplacian compare to python with {t_type} for dimensions {dimensions}: {eps2}")
        assert eps2 < 1e-30
        ...
        laplace_fermop = gpt.qcd.fermion.covariant_laplacian(u_unit, dimensions=dimensions, boundary_phases=[1.0, 1.0, 1.0, -1.0])

        # apply laplacian
        dst_fermop @= gpt(laplace_fermop * src)
        eps2 = gpt.norm2(sum([2.0 * (np.cos(p[ii]) - 1.0) for ii in dimensions]) * src - dst_fermop) / gpt.norm2(dst_fermop)
        gpt.message(f"Laplacian compare to exp with {t_type} for dimensions {dimensions}: {eps2}")
        assert eps2 < 1e-30
