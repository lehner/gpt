...
try:
    ...
    evec, ev_cheb = irl(
        c(w.NDagN), start, g.checkpointer("/hpcgpfs01/scratch/clehner/checkpointOdd")
    )
...
try:
    coarse_evec, coarse_ev = g.load(
        "/hpcgpfs01/scratch/clehner/coarse_basis", {"grids": grid_coarse}
    )
except g.LoadError:
    # generate eigenvectors
    coarse_evec, coarse_ev = irl(
        cop, cstart, g.checkpointer("/hpcgpfs01/scratch/clehner/checkpoint4")
    )
    g.save("/hpcgpfs01/scratch/clehner/coarse_basis", (coarse_evec, coarse_ev))

# smoother
smoother = g.algorithms.iterative.cg({"eps": 1e-8, "maxiter": 10})
n_smoother_iter = 3
...
try:
    ...
    for i, v in enumerate(coarse_evec):
        ...
        ev_smooth = g.algorithms.approx.evals(w.NDagN, [v_fine], check_eps2=1e-5, real=True)
        ev3[i] = ev_smooth[0]
...
solver = g.algorithms.approx.coarse_deflate(
    g.algorithms.iterative.cg({"eps": 1e-8, "maxiter": 1000}),
    coarse_evec[0 : len(basis)],
    basis,
    ev3[0 : len(basis)],
)
v_fine[:] = 0
