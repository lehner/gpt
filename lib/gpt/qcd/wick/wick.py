...
def coordinate_tensor(indices, b, f):
    ...
    t[:] = [f([(i // n_stride[j]) % (n_stride[j+1]//n_stride[j]) for j in range(len(n_stride)-1)]) for i in range(n_stride[-1])]
    return t
...
class wick:
        ...
        def __call__(self, x, spin_index, color_index):
            ...
            def _eval(context, path):
                ...
                if not self.is_bar:
                    ...
                    t = coordinate_tensor(propagators_indices, [alpha,a], lambda i: {(i[i_alpha],i[i_a]) : 1.0})
                    ...
                    t = coordinate_tensor(propagators_indices, [beta,b], lambda i: {
            ...
            def _contract(context, path):
                context.register_field(self.index, self.is_bar, path, [x], [spin_index, color_index])

            return expression([spin_index,color_index], _contract, _eval)

    def fermion(self, propagators):
        return wick.field(
            self.index(self.fermions, 1, -1),
            propagators,
            False
        )
    ...
    def sum(self, *arguments):
        ...
        def _eval(context, path):
            ...
            ev = [exp.evaluate(context, path + "/sum" + str(ii)) for ii, exp in enumerate(expressions)]
