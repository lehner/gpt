...
class base:
    def cayley_hamilton_exponential_and_jacobian(self, iQ, gradient_prime):

        if iQ.otype.shape[0] == 3:
            return self.cayley_hamilton_exponential_and_jacobian_3(iQ, gradient_prime)

        raise NotImplementedError()

    def cayley_hamilton_exponential_and_jacobian_3(self, iQ, gradient_prime):
        ...
        r11 = g(
            ...
            + emiu
            * (
                (-2.0 * cosw + (3.0 * u2 - w2) * xi0)
                + 1j * ((2.0 * u * cosw + 6.0 * u * xi0))
            )
        )
        ...
        r02 = g(
            -2.0 * e2iu
            + emiu * (-8.0 * u2 * xi0 + 1j * (2.0 * u * (cosw + xi0 + 3.0 * u2 * xi1)))
        )
