...
#    cg5 = charge_conjugation() * gpt.gamma[5]
#    unity = gpt.ot_matrix_spin_color(4, 3).identity()  # TODO brauchts des?
#    di_quark = quark_contract_13(gpt.eval(prop_down * cg5), gpt.eval(cg5 * prop_up))
#    return gpt.trace(
#        gpt.eval(pol_matrix * gpt.color_trace(prop_up * unity * gpt.spin_trace(di_quark))) +
#        gpt.eval(pol_matrix * gpt.color_trace(prop_up * di_quark))
#    )


def contract_neutron(prop_up, prop_down, pol_matrix):
    # return contract_proton(prop_down, prop_up, pol_matrix)
    cg5 = charge_conjugation() * gpt.gamma[5]
...
def contract_xi_zero(prop_up, prop_strange, pol_matrix):
    # return contract_proton(prop_strange, prop_up, pol_matrix)
    cg5 = charge_conjugation() * gpt.gamma[5]
    return baryon_base_contraction(prop_up, prop_up, prop_strange, pol_matrix, cg5)


def contract_xi_minus(prop_down, prop_strange, pol_matrix):
    # return contract_proton(prop_strange, prop_down, pol_matrix)
    cg5 = charge_conjugation() * gpt.gamma[5]
    return baryon_base_contraction(prop_down, prop_down, prop_strange, pol_matrix, cg5)


def contract_sigma_plus(prop_up, prop_strange, pol_matrix):
    # return contract_proton(prop_strange, prop_up, pol_matrix)
    cg5 = charge_conjugation() * gpt.gamma[5]
    return baryon_base_contraction(prop_strange, prop_up, prop_up, pol_matrix, cg5)


def contract_sigma_minus(prop_down, prop_strange, pol_matrix):
    return contract_proton(prop_strange, prop_down, pol_matrix)


def contract_sigma_zero(prop_up, prop_down, prop_strange, pol_matrix):  # TODO check!
    cg5 = charge_conjugation() * gpt.gamma[5]
    return baryon_base_contraction(prop_up, prop_down, prop_strange, pol_matrix, cg5)
    # spin_matrix = charge_conjugation() * gpt.gamma[5]
    # unity = gpt.ot_matrix_spin_color(4, 3).identity()
    # di_quark = quark_contract_13(gpt.eval(src_3 * spin_matrix), gpt.eval(spin_matrix * src_2))
    # return gpt.trace(
    #     gpt.eval(pol_matrix * gpt.color_trace(src_1 * unity * gpt.spin_trace(di_quark)))
    # ) + gpt.trace(
    #     gpt.eval(pol_matrix * gpt.color_trace(src_1 * di_quark))
    # )
