...
def map_pos(grid, cb, key):
    ...
    assert(all([ 0 <= top[i] and top[i] <= bottom[i] and bottom[i] <= grid.fdimensions[i] for i in range(nd) ]))
    return cgpt.coordinates_from_cartesian_view(top,bottom,grid.cb.cb_mask,cb.tag,"lexicographic")
...
def map_tidx_and_shape(l, key):
    ...
    if key is None:
        tidx=cgpt.coordinates_from_cartesian_view([0]*nd,list(shape),[0]*nd,gpt.none.tag,"reverse_lexicographic")
        return tidx, shape
    ...
    tidx=cgpt.coordinates_from_cartesian_view(top,bottom,[0]*nd,gpt.none.tag,"reverse_lexicographic")
...
def map_key(target,key):
    ...
    assert(all([ x.grid is grid for x in target[1:] ]) and 
           all([ cb.tag == x.checkerboard().tag for x in target[1:] ]))
