...
struct cgpt_stencil_matrix_matrix_vector_code_offload_t {
  int target;
  int accumulate;
  int source;
  int source_point;
  ComplexD weight;
  int size;
  cgpt_stencil_matrix_vector_factor_t* factor_1, * factor_2;
};

struct cgpt_stencil_matrix_matrix_vector_code_t {
  ...
  std::vector<cgpt_stencil_matrix_vector_factor_t> factor_1, factor_2; 
};

class cgpt_stencil_matrix_matrix_vector_base {
 public:
  virtual ~cgpt_stencil_matrix_matrix_vector_base() {};
  virtual void execute(const std::vector<cgpt_Lattice_base*>& matrix_1_fields,
		       const std::vector<cgpt_Lattice_base*>& matrix_2_fields,
		       const std::vector<cgpt_Lattice_base*>& vector_fields) = 0;
};

// TODO: below
template<typename M1, typename M2, typename V>
class cgpt_stencil_matrix_matrix_vector : public cgpt_stencil_matrix_matrix_vector_base {
 public:
