...
for u in U:
    ...
    g.message("SU violation",g.norm2(g.matrix.det(u) - g.identity(g.complex(u.grid))) / g.norm2(u))
...
for t in range(Nt):
    ...
    lap = g.create.smear.laplace(
        g.covariant.shift(U3_t, boundary_phases=[1.0, 1.0, 1.0, -1.0]), dimensions=[0,1,2]
    )
    ...
    if t == 0:
        g.message("Power iteration spectrum test",g.algorithms.eigen.power_iteration(eps=1e-7,maxiter=200)(slap, start))
