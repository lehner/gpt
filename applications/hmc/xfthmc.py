...
pt_e = [
    g.qcd.gauge.smear.local_stout(rho=0.124, dimension=mu, checkerboard=g.even) for mu in range(4)
]

pt_o = [
    g.qcd.gauge.smear.local_stout(rho=0.124, dimension=mu, checkerboard=g.odd) for mu in range(4)
]

for j in range(4):
    test_U = pt_e[j].inv(pt_e[j](U))
    for i in range(4):
        eps = g.norm2(test_U[i] - U[i]) / g.norm2(U[i])
        g.message(f"Test inversion (e{j}{i}): {eps}")
        assert eps < 1e-25
    test_U = pt_o[j].inv(pt_o[j](U))
    for i in range(4):
        eps = g.norm2(test_U[i] - U[i]) / g.norm2(U[i])
        g.message(f"Test inversion (e{j}{i}): {eps}")
        assert eps < 1e-25
...
for i in reversed(range(4)):
    ...
    if a_log_det is None:
        a_log_det = pt_e[i].action_log_det_jacobian()
    else:
        a_log_det = a_log_det.transformed(pt_e[i]) + pt_e[i].action_log_det_jacobian()
...
def hamiltonian():
    return a0(p_mom) + a1(U) + a_log_det(U)

# molecular dynamics
sympl = g.algorithms.integrator.symplectic

def add(X, Y):
    return [g(x + y) for x, y in zip(X, Y)]
