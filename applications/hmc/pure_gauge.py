...
# metropolis
metro = g.algorithms.markov.metropolis(rng, mdint, h, U)
...
for i in range(1,11):
    ...
    g.message(f'Plaquette = {g.qcd.gauge.plaquette(U)}, Acceptance = {numpy.mean(h[:,0]):.2f}, |dH| = {numpy.mean(numpy.abs(h[:,1])):.4e}')
