...
f_lap = g.qcd.gauge.algebra_laplace_polynomial(
    U,
    1.0, # scale
    [-0.1,-0.2,-0.3] # masses
)
...
f_lap_U = g.matrix_operator(mat=f_lap, inv_mat=inv_f_lap, accept_list=True, accept_guess=(False, True))
...
aP_unsmeared = g.qcd.scalar.action.general_mass_term(inv_M=f_lap_U_sqr, sqrt_inv_M=f_lap_U, inv_M_projected_gradient=f_lap_sqr_projected_gradient)

# R-action can be done less frequently and can be inverse of polynomial
aR_unsmeared = g.qcd.scalar.action.general_mass_term(inv_M=f_lap_U_sqr.inv(), sqrt_inv_M=f_lap_U.inv(), inv_M_projected_gradient=inv_f_lap_sqr_projected_gradient)

# use smeared gauge links in mass matrix
sm = g.qcd.gauge.smear.stout(rho=0.12)
aP = aP_unsmeared.transformed(sm, indices=[0,1,2,3])
aR = aR_unsmeared.transformed(sm, indices=[0,1,2,3])
...
_ip_inner_imp = sympl.update_p(p_mom, lambda: add(aQ.gradient(U, U), aP.gradient(U + p_mom2, U)), "P_inner")
...
integrator = sympl.OMF2(
    nouter, ip_outer_imp,
    sympl.OMF2(ninner, ip_inner_imp, iq_imp)
)
...
h1 = hamiltonian()
integrator(-tau)
eps2 = sum([g.norm2(u - u2) for u, u2 in zip(U + p_mom, U0 + p_mom0)]) / sum([g.norm2(u) for u in U0 + p_mom0])
...
def hmc():
    U_sm = sm(U)
    aP_unsmeared.draw(U_sm + p_mom, rng)
    aR_unsmeared.draw(U_sm + r_mom, rng)
    accrej = metro(U)
    h0 = hamiltonian()
    integrator(tau)
    h1 = hamiltonian()
